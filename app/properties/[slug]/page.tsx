...
export default async function PropertyDetailsPage({
  ...
  const propertyType = (property.type as string)?.toLowerCase();
  const translatedType = (dict.common.types as Record<string, string>)[propertyType] || property.type;
            ...
            <div className="relative aspect-[16/10] w-full overflow-hidden rounded-xl shadow-sm group bg-slate-200">
              ...
              {images && images.length > 1 && (
                <button className="absolute bottom-4 right-4 bg-white/90 hover:bg-white text-nordic-dark px-4 py-2 rounded-lg text-sm font-medium shadow-lg backdrop-blur transition-all flex items-center gap-2">
                  ...
                  {dict.propertyDetails.viewAllPhotos} ({images.length})
                </button>
              )}
            </div>
            ...
            <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic-dark/50">{dict.propertyDetails.squareMeters}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic-dark/50">{dict.propertyDetails.bedrooms}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic-dark/50">{dict.propertyDetails.bathrooms}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  <span className="material-icons text-mosque text-2xl mb-2">home</span>
                  <span className="text-xl font-bold text-nordic-dark text-center">{translatedType}</span>
                  <span className="text-xs uppercase tracking-wider text-nordic-dark/50">{dict.propertyDetails.type}</span>
                </div>
              </div>
            </div>
            <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
              <h2 className="text-lg font-semibold mb-4 text-nordic-dark">
                {dict.propertyDetails.aboutThis.replace("{type}", translatedType.toLowerCase())}
              </h2>
              <div className="prose prose-slate max-w-none text-nordic-dark/70 leading-relaxed">
                <p className="mb-4">
                  Experience modern luxury in this stunning {translatedType.toLowerCase()} located in {property.location}. Designed with an emphasis on indoor-outdoor living, the residence features high-end finishes that flood the interiors with natural light.
                </p>
                <p>
              ...
              <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-8">
                  {property.amenities.map((amenity: string, i: number) => {
                    const dictKey = amenityMapping[amenity];
                    const label = dictKey ? (dict.amenities as Record<string, string>)[dictKey] : amenity;
                    return (
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-nordic-dark/60">
                    {dict.propertyDetails.mortgageStarting
                      .replace("{amount}", `${property.currency}${(property.price / 360).toFixed(0)}`)}
                  </p>
                </div>
              </div>
